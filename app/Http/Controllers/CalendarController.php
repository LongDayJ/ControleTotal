...
class CalendarController extends Controller
{
	public function index(Request $request)
	{
			...
			return view('schedule.index', 
					compact('pacientes', 'medicos', 'events', 'procedimentos'));
	}
	...
	public function destroy($id)
	{
			...
			if ($event) {
					if ($event->data < now()->toDateString()) {
							return redirect()->route('agendamento.index')->with('error', 'Não é possível excluir um evento que já passou.');
					}
					$event->delete();
					return redirect()->route('agendamento.index')->with('success', 'Evento excluído com sucesso.');
			} else {
					return redirect()->route('agendamento.index')->with('error', 'Evento não encontrado.');
			}
	}
}
